...
public sealed class FileScopedNamespaceDetector : IPatternDetector
{
    ...
    public Task<IReadOnlyList<DetectionResult>> DetectAsync(DetectionContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (namespaceDeclarations.Count != 1) { return Task.FromResult<IReadOnlyList<DetectionResult>>(results); }

        var ns = namespaceDeclarations[0];

        if (ns.DescendantNodes().OfType<NamespaceDeclarationSyntax>().Any()) { return Task.FromResult<IReadOnlyList<DetectionResult>>(results); }
    ...
    private static string Outdent(string text) => Regex.Replace(text, "(?m)^(    |\t)", string.Empty);
}
