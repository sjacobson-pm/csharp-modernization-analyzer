...
public sealed class PrimaryConstructorDetector : IPatternDetector
{
    ...
    public Task<IReadOnlyList<DetectionResult>> DetectAsync(DetectionContext context, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var classDeclaration in root.DescendantNodes().OfType<ClassDeclarationSyntax>())
        {
            ...
            if (constructor.ParameterList.Parameters.Count == 0 ||
                ...
                constructor.Initializer is not null) { continue; }

            if (!TryMatchAssignments(classDeclaration, constructor, context, cancellationToken, out var fieldAssignments)) { continue; }
            ...
            var suggestedCode =
                $"{classHeader}{Environment.NewLine}{{{Environment.NewLine}{string.Join(Environment.NewLine, fieldLines)}{Environment.NewLine}}}";
        ...
        out IReadOnlyList<FieldAssignment> fieldAssignments)
    {
        ...
        if (constructor.Body is null || constructor.Body.Statements.Count != constructor.ParameterList.Parameters.Count) { return false; }
        ...
        {
            if (!DetectorUtilities.TryGetSimpleAssignment(statement, out var assignment)) { return false; }
            ...
            if (fieldSymbol is null || parameterSymbol is null) { return false; }

            if (!SymbolEqualityComparer.Default.Equals(
                    fieldSymbol.ContainingType,
                    context.SemanticModel.GetDeclaredSymbol(classDeclaration, cancellationToken))) { return false; }

            if (!fieldSymbol.IsReadOnly) { return false; }

            if (!SymbolEqualityComparer.Default.Equals(fieldSymbol.Type, parameterSymbol.Type)) { return false; }

            if (!assignedFields.Add(fieldSymbol) || !assignedParameters.Add(parameterSymbol)) { return false; }

            if (fieldSymbol.DeclaringSyntaxReferences.FirstOrDefault()?.GetSyntax(cancellationToken) is not VariableDeclaratorSyntax
                {
                    Parent: VariableDeclarationSyntax { Variables.Count: 1, Parent: FieldDeclarationSyntax fieldDeclaration } variableDeclaration,
                } declarator) { return false; }
