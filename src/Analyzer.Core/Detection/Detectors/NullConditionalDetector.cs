...
public sealed class NullConditionalDetector : IPatternDetector
{
    ...
    public Task<IReadOnlyList<DetectionResult>> DetectAsync(DetectionContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (!prefersNullPropagation && !prefersCoalesce) { return Task.FromResult<IReadOnlyList<DetectionResult>>(results); }

        var root = context.SyntaxTree.GetRoot(cancellationToken);

        if (prefersNullPropagation) { this.DetectNullPropagation(root, context, cancellationToken, results); }

        if (prefersCoalesce) { this.DetectCoalesce(root, context, cancellationToken, results); }

        return Task.FromResult<IReadOnlyList<DetectionResult>>(results);
    }

    private void DetectNullPropagation(SyntaxNode root, DetectionContext context, CancellationToken cancellationToken, List<DetectionResult> results)
    {
        foreach (var ifStatement in root.DescendantNodes().OfType<IfStatementSyntax>())
        {
            if (ifStatement.Else is not null) { continue; }

            if (!TryMatchNullCheck(ifStatement.Condition, SyntaxKind.NotEqualsExpression, out var target)) { continue; }

            if (!DetectorUtilities.TryGetSingleStatement(ifStatement.Statement, out var bodyStatement)) { continue; }

            if (bodyStatement is not ExpressionStatementSyntax { Expression: InvocationExpressionSyntax invocation }) { continue; }

            if (invocation.Expression is not MemberAccessExpressionSyntax memberAccess) { continue; }

            if (!DetectorUtilities.ExpressionsMatch(target, memberAccess.Expression, context.SemanticModel, cancellationToken)) { continue; }
    ...
    private void DetectCoalesce(SyntaxNode root, DetectionContext context, CancellationToken cancellationToken, List<DetectionResult> results)
    {
        foreach (var conditional in root.DescendantNodes().OfType<ConditionalExpressionSyntax>())
        {
            if (conditional.Condition is not BinaryExpressionSyntax condition) { continue; }
            ...
            if (target is null || fallback is null) { continue; }
    ...
    private static bool TryMatchNullCheck(ExpressionSyntax expression, SyntaxKind kind, out ExpressionSyntax target)
    {
        ...
        if (expression is not BinaryExpressionSyntax condition || !condition.IsKind(kind)) { return false; }
