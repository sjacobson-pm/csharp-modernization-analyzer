...
public sealed class UsingDeclarationDetector : IPatternDetector
{
    ...
    public Task<IReadOnlyList<DetectionResult>> DetectAsync(DetectionContext context, CancellationToken cancellationToken = default)
    {
        ...
        {
            if (usingStatement.Declaration is null || usingStatement.Expression is not null)
                continue;

            if (usingStatement.Parent is not BlockSyntax parentBlock)
                continue;

            if (parentBlock.Statements.LastOrDefault() != usingStatement)
                continue;

            if (usingStatement.Statement is not BlockSyntax bodyBlock)
                continue;
