...
public sealed class SwitchExpressionDetector : IPatternDetector
{
    ...
    public Task<IReadOnlyList<DetectionResult>> DetectAsync(DetectionContext context, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var ifStatement in root.DescendantNodes().OfType<IfStatementSyntax>())
        {
            ...
            if (!TryBuildSuggestion(ifStatement, context, cancellationToken, out var suggestedCode))
                continue;
        ...
        out string suggestedCode)
    {
        ...
        ExpressionSyntax? defaultResult = null;
        ...
        while (true)
        {
            ...
            if (!TryGetBranchResult(current.Statement, ref mode, ref assignmentTarget, context.SemanticModel, cancellationToken, out var resultExpression))
                return false;
            ...
            if (!TryGetBranchResult(current.Else.Statement, ref mode, ref assignmentTarget, context.SemanticModel, cancellationToken, out defaultResult))
                return false;

            break;
        }

        if (branches.Count < 3 || discriminant is null || defaultResult is null || mode == BranchMode.Unknown)
            return false;
        ...
        out ExpressionSyntax resultExpression)
    {
        ...
        if (!DetectorUtilities.TryGetSingleStatement(statement, out var singleStatement))
            return false;

        if (singleStatement is ReturnStatementSyntax { Expression: { } returnExpression })
        {
            if (mode == BranchMode.Assignment)
                return false;
        ...
        else if (!DetectorUtilities.ExpressionsMatch(assignmentTarget, assignment.Left, semanticModel, cancellationToken))
        {
            return false;
        }
