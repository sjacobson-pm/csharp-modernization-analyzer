...
public sealed class VarUsageDetector : IPatternDetector
{
    ...
    public Task<IReadOnlyList<DetectionResult>> DetectAsync(DetectionContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (!prefersBuiltIn && !prefersApparent && !prefersElsewhere) { return Task.FromResult<IReadOnlyList<DetectionResult>>(results); }
        ...
        {
            if (localDeclaration.IsConst || localDeclaration.UsingKeyword != default) { continue; }
            ...
            if (DetectorUtilities.IsNullLiteral(initializer)) { continue; }
            ...
            if (declaredType is null || initializerType is null) { continue; }

            if (!SymbolEqualityComparer.Default.Equals(declaredType, initializerType)) { continue; }

            if (declaredType.TypeKind == TypeKind.Error || initializerType.TypeKind == TypeKind.Error) { continue; }
    ...
    {
        if (declaredTypeSyntax is PredefinedTypeSyntax) { return true; }

        return declaredType.SpecialType is SpecialType.System_Boolean or SpecialType.System_Byte or SpecialType.System_Char
            or SpecialType.System_Decimal or SpecialType.System_Double or SpecialType.System_Int16 or SpecialType.System_Int32
            or SpecialType.System_Int64 or SpecialType.System_Object or SpecialType.System_SByte or SpecialType.System_Single
            or SpecialType.System_String or SpecialType.System_UInt16 or SpecialType.System_UInt32 or SpecialType.System_UInt64;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (initializer is ObjectCreationExpressionSyntax or ArrayCreationExpressionSyntax or ImplicitArrayCreationExpressionSyntax) { return true; }
