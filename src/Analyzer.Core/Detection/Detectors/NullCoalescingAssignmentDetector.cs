...
public sealed class NullCoalescingAssignmentDetector : IPatternDetector
{
    ...
    public Task<IReadOnlyList<DetectionResult>> DetectAsync(DetectionContext context, CancellationToken cancellationToken = default)
    {
        ...
        this.DetectIfAssignments(root, context, cancellationToken, results);
        this.DetectCoalesceAssignments(root, context, cancellationToken, results);

        return Task.FromResult<IReadOnlyList<DetectionResult>>(results);
    }

    private void DetectIfAssignments(SyntaxNode root, DetectionContext context, CancellationToken cancellationToken, List<DetectionResult> results)
    {
        foreach (var ifStatement in root.DescendantNodes().OfType<IfStatementSyntax>())
        {
            if (ifStatement.Else is not null) { continue; }

            if (!TryGetNullCheckedTarget(ifStatement.Condition, out var target)) { continue; }

            if (!DetectorUtilities.TryGetSimpleAssignment(ifStatement.Statement, out var assignment)) { continue; }

            if (!DetectorUtilities.ExpressionsMatch(target, assignment.Left, context.SemanticModel, cancellationToken)) { continue; }
        ...
        List<DetectionResult> results)
    {
        foreach (var assignment in root.DescendantNodes().OfType<AssignmentExpressionSyntax>())
        {
            ...
            if (assignment.Right is not BinaryExpressionSyntax { RawKind: (int)SyntaxKind.CoalesceExpression } coalesce) { continue; }

            if (!DetectorUtilities.ExpressionsMatch(assignment.Left, coalesce.Left, context.SemanticModel, cancellationToken)) { continue; }
