...
public sealed class PatternMatchingDetector : IPatternDetector
{
    ...
    public Task<IReadOnlyList<DetectionResult>> DetectAsync(DetectionContext context, CancellationToken cancellationToken = default)
    {
        ...
        DetectIsWithCast(root, context, results);
        DetectAsWithNullCheck(root, context, results, cancellationToken);
    ...
    private void DetectIsWithCast(SyntaxNode root, DetectionContext context, List<DetectionResult> results)
    {
        ...
        {
            if (ifStatement.Condition is not BinaryExpressionSyntax isExpr || !isExpr.IsKind(SyntaxKind.IsExpression))
                continue;

            if (isExpr.Right is not TypeSyntax typeSyntax)
                continue;
            ...
            var updatedIf = ifStatement
                .WithCondition(SyntaxFactory.ParseExpression($"{isExpr.Left} is {typeSyntax} {variableName}"))
    ...
    {
        if (context.Standards.IsStyleDisabled("csharp_style_pattern_matching_over_as_with_null_check"))
            return;
        ...
        foreach (var localDecl in localDeclarations)
        {
            ...
            if (variable?.Initializer?.Value is not BinaryExpressionSyntax asExpr || !asExpr.IsKind(SyntaxKind.AsExpression))
                continue;

            if (asExpr.Right is not TypeSyntax typeSyntax)
                continue;
            ...
            if (declIndex < 0 || declIndex + 1 >= siblings.Count)
                continue;
            ...
            if (!IsNullCheckForVariable(ifStatement.Condition, varName))
                continue;
            ...
            var updatedIf = ifStatement
                .WithCondition(SyntaxFactory.ParseExpression($"{asExpr.Left} is {typeSyntax} {varName}"))
                .NormalizeWhitespace()
    ...
    private static StatementSyntax RewriteIsPatternBody(StatementSyntax statement, string sourceExpression, string typeName, string variableName)
    {
        ...
        if (rewritten is not BlockSyntax block)
            return rewritten;
