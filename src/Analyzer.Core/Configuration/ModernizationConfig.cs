...
public sealed class ModernizationConfig
{
    ...
    public bool IsExcluded(string filePath)
    {
        ...
        if (this.IncludePatterns.Count > 0) { return !this.IncludePatterns.Any(p => MatchesGlob(normalized, p)); }

        return false;
    }

    /// <summary>Check if a rule is enabled (default: true unless explicitly disabled).</summary>
    public bool IsRuleEnabled(string ruleId) => !this.RuleOverrides.TryGetValue(ruleId, out var ruleOverride) || ruleOverride.Enabled;
    ...
    {
        if (this.RuleOverrides.TryGetValue(ruleId, out var ruleOverride) && ruleOverride.SeverityOverride is not null)
        {
            return Enum.TryParse<Detection.Severity>(ruleOverride.SeverityOverride, true, out var severity) ? severity : null;
        }
    ...
    {
        if (!File.Exists(filePath)) { return new ModernizationConfig(); }
    ...
    public static ModernizationConfig LoadFromYaml(string yaml)
    {
        ...
        catch { return new ModernizationConfig(); }
    }

    private static ModernizationConfig MapFromYaml(YamlConfigRoot? root)
    {
        if (root is null) { return new ModernizationConfig(); }
        ...
        var config = new ModernizationConfig
        {
            ...
            ExternalStandardUrls =
                root.Standards?.External?.Select(static e => e.Url)
        ...
        if (root.Rules is not null)
        {
            ...
            {
                config.RuleOverrides[ruleId] = new RuleOverride { Enabled = ruleConfig.Enabled ?? true, SeverityOverride = ruleConfig.Severity };
            }
        }
    ...
    private static bool MatchesGlob(string path, string pattern)
    {
        ...
        var regexBody = System.Text.RegularExpressions.Regex.Escape(normalizedPattern).Replace("\\*\\*", ".*").Replace("\\*", "[^/]*");

        var regexPattern = normalizedPattern.StartsWith("**", StringComparison.Ordinal) ? $"^{regexBody}$" : $"^(?:.*?/)?{regexBody}$";
