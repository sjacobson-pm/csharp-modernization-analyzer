...
public sealed class IntentParser
{
    public CopilotIntent Parse(string? message)
    {
        ...
        if (string.IsNullOrWhiteSpace(rawMessage)) { return new UnknownIntent(rawMessage); }

        var normalized = Normalize(rawMessage);

        if (IsListRulesIntent(normalized)) { return new ListRulesIntent(rawMessage); }

        var explainMatch = Regex.Match(normalized, @"\b(?:explain|describe|what does)\s+(mod\d{3})\b", RegexOptions.IgnoreCase);

        if (explainMatch.Success) { return new ExplainRuleIntent(rawMessage, explainMatch.Groups[1].Value.ToUpperInvariant()); }

        if (Regex.IsMatch(normalized, @"\bscan\s+(?:this|the|current)?\s*pr\b", RegexOptions.IgnoreCase)) { return new ScanPrIntent(rawMessage); }

        if (Regex.IsMatch(normalized, @"\bwhat can be modernized in this file\b", RegexOptions.IgnoreCase) ||
            Regex.IsMatch(normalized, @"\bscan\s+(?:this|the)\s+file\b", RegexOptions.IgnoreCase)) { return new ScanFileIntent(rawMessage, null); }

        var explicitPath = ExtractPath(rawMessage);

        if (!string.IsNullOrWhiteSpace(explicitPath))
        {
            if (LooksLikeFilePath(explicitPath)) { return new ScanFileIntent(rawMessage, explicitPath); }
    ...
    private static bool IsListRulesIntent(string normalized) =>
        normalized.Contains("what rules are available", StringComparison.OrdinalIgnoreCase) ||
    ...
    private static string Normalize(string message) => Regex.Replace(message.Trim(), @"\s+", " ");

    private static string? ExtractPath(string message)
    {
        var quotedMatch = Regex.Match(message, @"\bscan\s+['\""](?<path>[^'\""\r\n]+)['\""]", RegexOptions.IgnoreCase);

        if (quotedMatch.Success) { return quotedMatch.Groups["path"].Value.Trim(); }

        var inlineMatch = Regex.Match(message, @"\bscan\s+(?<path>(?!this\b|the\b|current\b|pr\b)[^\r\n]+)", RegexOptions.IgnoreCase);

        if (!inlineMatch.Success) { return null; }

        var path = inlineMatch.Groups["path"].Value.Trim().TrimEnd('.', '?', '!', ';', ':');
