...
internal sealed class ScanHandler(IHttpClientFactory httpClientFactory)
{
    ...
    public Task<CopilotResponse> HandleAsync(ScanPrIntent intent, CopilotRequestContext requestContext, CancellationToken cancellationToken)
    {
        ...
        var prLabel = requestContext.PullRequestNumber is { } pullRequestNumber
        ...
        return Task.FromResult(new CopilotResponse("Scanning...", "Analyzing files for modernization opportunities", markdown, []));
    }
    ...
    public Task<CopilotResponse> HandleAsync(ScanDirectoryIntent intent, CopilotRequestContext requestContext, CancellationToken cancellationToken) =>
        this.BuildScanResponseAsync(intent.DirectoryPath, "directory", cancellationToken);
    ...
    public Task<CopilotResponse> HandleAsync(ScanFileIntent intent, CopilotRequestContext requestContext, CancellationToken cancellationToken)
    {
        ...
        return this.BuildScanResponseAsync(requestedPath, "file", cancellationToken);
    }
    ...
    private async Task<CopilotResponse> BuildScanResponseAsync(string? requestedPath, string scope, CancellationToken cancellationToken)
    {
        ...
        if (resolvedPath is null) { return this.BuildPlaceholderScanResponse(requestedPath, scope); }

        var localScanResult = await this.TryAnalyzeAsync(repoRoot, resolvedPath, cancellationToken);

        var references = localScanResult.Files.Select(file => new CopilotReference("file", ToReferencePath(repoRoot, file))).ToList();
    ...
    private CopilotResponse BuildPlaceholderScanResponse(string requestedPath, string scope)
    {
        ...
        return new CopilotResponse("Scanning...", "Analyzing files for modernization opportunities", markdown, references);
    }
    ...
    private async Task<LocalScanResult> TryAnalyzeAsync(string repoRoot, string resolvedPath, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (files.Count == 0) { return new LocalScanResult(false, files, [], note ?? "No C# files matched the request."); }

            var standardsResolver = new StandardsResolver(httpClientFactory.CreateClient());
        ...
        catch (Exception ex) { return new LocalScanResult(false, [], [], $"Roslyn analysis is not available yet in webhook mode: {ex.Message}"); }
    }
    ...
    private static string BuildScanMarkdown(string scope, string requestedPath, string repoRoot, LocalScanResult result)
    {
        ...
        builder.AppendLine(
            $"Found **{result.Detections.Count}** modernization opportunit{(result.Detections.Count == 1 ? "y" : "ies")} across **{result.Detections.Select(detection => detection.FilePath).Distinct(StringComparer.OrdinalIgnoreCase).Count()}** file(s).");

        builder.AppendLine();
        builder.AppendLine("### Findings");

        foreach (var detection in result.Detections.OrderBy(detection => detection.FilePath, StringComparer.OrdinalIgnoreCase)
    ...
    private static List<string> DiscoverFiles(string path, ModernizationConfig config, out string? note)
    {
        ...
        if (File.Exists(path) && path.EndsWith(".cs", StringComparison.OrdinalIgnoreCase)) { return [path]; }
    ...
    private static IReadOnlyList<MetadataReference> BuildMetadataReferences() =>
        new[] { typeof(object).Assembly, typeof(Console).Assembly, typeof(Enumerable).Assembly, typeof(List<>).Assembly, typeof(Task).Assembly }
    ...
    private static string? ResolveLocalPath(string repoRoot, string requestedPath)
    {
        ...
        {
            return File.Exists(cleanedPath) || Directory.Exists(cleanedPath) ? Path.GetFullPath(cleanedPath) : null;
        }
    ...
    private sealed record LocalScanResult(bool Executed, IReadOnlyList<string> Files, IReadOnlyList<DetectionResult> Detections, string? Note);
}
